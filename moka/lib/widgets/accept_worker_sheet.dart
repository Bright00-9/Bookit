...
class _AcceptWorkerSheetState extends State<AcceptWorkerSheet> {
  ...
  Future<void> _handlePayAndAccept() async {
    ...
    try {
      ...
      final txData = await PaystackService.initializeTransaction(
        email: user.email!,
        amountGhc: _fee,
        reference: reference,
        metadata: {
      ...
      await _feeService.createFeeRecord(
      ...
      if (result != true) {
        await _feeService.markFeeFailed(
          applicationId: widget.applicationId,
          paystackReference: reference,
        );
        if (mounted) {
      ...
      if (verification['status'] == 'success') {
        await _feeService.confirmPaymentAndAccept(
          applicationId: widget.applicationId,
          paystackReference: reference,
          paystackTransactionId: verification['id'].toString(),
        );
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _isProcessing
                    ...
                    : _feePaid
                        ? const Text('✅ Already Paid — Accepted')
                        : Text(
